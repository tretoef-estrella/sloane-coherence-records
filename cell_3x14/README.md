...
- A basin-floor benchmark for the (3, 14) coherence landscape that future work can use as a target or as a reference for paradigm comparison.
- A structural observation about the basin floor of this cell (degenerate critical structure with multiple critical points at the same μ value to within one ULP binary64) which may itself be of independent interest.

The Mixon holder has stood for over six years and remains the only published baseline for the cell. This submission improves on it by the minimum amount the Game of Sloanes rule will accept, and explicitly does not claim more than that.
...
The distinctive feature of Sanjuanbautista relative to the parent repository's engines is that it operates in **quad-precision (128-bit) arithmetic throughout the inner loop**, rather than in IEEE-754 double-precision or in double-double. This makes the engine slower than Aquiles or Boagrius per iteration, but it allows the engine to resolve descent below the threshold at which conventional double-precision engines saturate. In cells where the holder configuration sits near a degenerate basin floor — as the Mixon (2019) holder for (3, 14) appears to — a quad-precision inner loop is the difference between stalling at the saturation threshold and being able to push to the basin floor itself.
...
- **[`3x14_holder_mixon.txt`](3x14_holder_mixon.txt)** — Reference copy of the Mixon (2019) catalog holder (file `3x14_dgm.txt` in the Game of Sloanes catalog), included here for direct side-by-side numerical comparison without requiring an external lookup.
- **[`RESULTS_3x14.md`](RESULTS_3x14.md)** — Headline numbers, five-kernel ratification tables for both the submission and the holder, bounds comparison (Welch, Levenshtein-2, Delsarte LP at degree 14), cushion analysis, and structural verification.
- **[`Paper_3x14_basin_floor.md`](Paper_3x14_basin_floor.md)** — Companion technical note documenting the basin-floor evidence (continuous descent trajectory, identical saturated-pair topology, multiple critical points at the basin floor, random-initialisation convergence) and the optimisation portfolio applied to the cell.
...
A reader can also recompute the holder for direct comparison:

```bash
python3 verify_sloane_independent.py cell_3x14/3x14_holder_mixon.txt
```
...
Unit-norm check: PASSED (within 1e-10).
Welch lower bound (d=3, n=14): 0.5310850045
Coherence mu(Phi) = 0.637630521755923
Worst pair: vectors (2, 3) with |<v_2, v_3>| = 0.637630521755923
```

**Note on the worst-pair indices.** The verifier above will report a stable worst pair `(2, 3)` for the Mixon holder across any reasonable double-precision kernel. For the submission, however, the worst-pair indices vary across independent verification kernels (see [`RESULTS_3x14.md`](RESULTS_3x14.md) §"A structural observation about the worst pair" and [`Paper_3x14_basin_floor.md`](Paper_3x14_basin_floor.md) §3.3). The coherence value `0.637630514941861` is reproduced byte-exact in all five kernels; the migrating argmax pair is a structural property of the basin floor for this cell, not a kernel error or a numerical artefact.
...
- **The packing files, results document, basin-floor technical note, and this README**: MIT License (see [`../LICENSE`](../LICENSE) and [`../LICENSE.md`](../LICENSE.md)).
...
- **Dustin G. Mixon** — for the published (2019) catalog entry for cell `(3, 14)` (file `3x14_dgm.txt` in the Game of Sloanes catalog) against which this submission is compared. The Mixon holder was the standing record for over six years before this submission, and its byte-exact unit-norm structure and saturated-pair topology are documented as the reference baseline throughout this folder.
